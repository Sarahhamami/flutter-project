...
class ActivityProvider with ChangeNotifier {
  ...
  Future<bool> addStepsFromRoute(int steps, double distance) async {
    _setLoading(true);
    try {
      // Add steps to SQLite database
      final success = await SqliteNutritionService.addSteps(steps, DateTime.now());
      if (success) {
        // Also update the in-memory activity data for immediate UI updates
        if (_todaySteps == null) {
          _todaySteps = DailySteps(date: DateTime.now(), steps: 0);
        }
