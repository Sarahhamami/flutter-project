...
class _DashboardPageState extends State<DashboardPage> {
              ...
              child: ModernCard(
                ...
                child: Column(
                  children: [
                    ...
                    Text(
                      'Debug: ${progress.caloriesConsumed}',
                      style: TextStyle(
                        fontSize: 10,
                        color: AppColors.grey,
                      ),
  ...
  Future<void> _addMeal() async {
    ...
    if (result != null && mounted) {
      ...
      if (success) {
        ...
        // Force refresh the dashboard to update the progression chart
  ...
  Future<void> _navigateToMapTracker() async {
    ...
    // Always refresh the dashboard data when returning from map tracker
    // to synchronize any modified data (steps added from route tracking)
    if (mounted) {
      await context.read<DashboardProvider>().initialize();

      // Show confirmation that data was synchronized
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Données d\'activité synchronisées !'),
